...
async function populate() {
  await Promise.all([
    sql`DROP TABLE IF EXISTS playing_with_neon`,
    sql`CREATE TABLE branches (branch_name TEXT, connection_string TEXT)`,
  ])
  ...
  createReadStream('./spotify_millsongdata.csv')
    ...
    .on('data', function (row) {
      counter += 1
      transactions.push(
        sql`INSERT INTO playing_with_neon (id, singer, song) VALUES (${counter}, ${slug(row[0])}, ${slug(row[1])})`
      )
    })
